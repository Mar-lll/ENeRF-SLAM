...
def get_sample_uv(H0, H1, W0, W1, n, depth, color, device='cuda:0'):
    ...
    if False:
        orb = cv2.ORB_create(nfeatures=10000,edgeThreshold=5)
        # 寻找关键点 
        img = cv2.cvtColor(((color.cpu().numpy())*255).astype(np.uint8), cv2.COLOR_BGR2RGB)
        kp1 = orb.detect(img)
        i=[]
        j =[]
        for keypoint in kp1:
            i.append(int(keypoint.pt[0]))
            j.append(int(keypoint.pt[1]))
        i = torch.tensor(i).to(device)
        j = torch.tensor(j).to(device)
        color = color[j,i].reshape(-1,3)
        depth = depth[j,i].reshape(-1)
