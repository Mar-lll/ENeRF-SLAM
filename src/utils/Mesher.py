            ...
            if show_forecast:
                ...
                for i, pnts in enumerate(
                        torch.split(forecast_points,
                                    ...
                                    dim=0)):
                    z_forecast.append(
                        self.eval_points(pnts, decoders,viewdirs, c, 'coarse',
                                         device).cpu().numpy()[:, -1])
                ...
                for i, pnts in enumerate(
                        torch.split(seen_points, self.points_batch_size,
                                    dim=0)):
                    z_seen.append(
                        self.eval_points(pnts,viewdirs, decoders, c, 'fine',
                                         device).cpu().numpy()[:, -1])
            ...
            if color:
                if self.color_mesh_extraction_method == 'direct_point_query':
                    ...
                    for i in range(0, rays_d.shape[0], batch_size):
                        ...
                        viewdirs = rays_d_batch
                        viewdirs = viewdirs / torch.norm(viewdirs, dim=-1, keepdim=True)
                        depth, uncertainty, color = self.renderer.render_batch_ray(
                            c, decoders, rays_d_batch, rays_o_batch,viewdirs, device, 
