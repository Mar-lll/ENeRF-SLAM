...
class Renderer(object):
    ...
    def eval_points(self, p,viewdirs, decoders, c=None, stage='color', device='cuda:0'):
        """
        ...
        if self.use_viewdirs:
            viewdirs_split = torch.split(viewdirs, self.points_batch_size)

        bound = self.bound
   
        rets = []
        if self.use_viewdirs:
            for pi,viewdir in zip(p_split,viewdirs_split):
    ...
    def render_batch_ray(self, c, decoders, rays_d, rays_o,viewdirs, device,stage ,  gt_depth=None):
        """
        ...
        if viewdirs is not None:
            ...
            viewdirs = torch.reshape(input_dirs, [-1, input_dirs.shape[-1]])
  
        pointsf = pts.reshape(-1, 3)
        viewdirs = viewdirs.reshape(-1,3)
     
        raw = self.eval_points(pointsf,viewdirs, decoders, c, stage, device)
        ...
        if N_importance > 0:
            ...
            if view_dirs2 is not None:
                input_dirs = view_dirs2[:,None].expand(pts.shape)
                view_dirs2 = torch.reshape(input_dirs, [-1, input_dirs.shape[-1]])
            pts = pts.reshape(-1, 3)
            viewdirs2 = view_dirs2.reshape(-1,3)
            raw = self.eval_points(pts,viewdirs2, decoders, c, stage, device)
    ...
    def render_img(self, c, decoders, c2w, device, stage, gt_depth=None):
        ...
        with torch.no_grad():
            ...
            for i in range(0, rays_d.shape[0], ray_batch_size):
                ...
                viewdirs_batch = viewdirs[i:i+ray_batch_size]
                if gt_depth is None:
    ...
    def regulation(self, c, decoders, rays_d, rays_o,viewdirs, gt_depth, device, stage='color'):
        """
