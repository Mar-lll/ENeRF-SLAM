...
class MLP(nn.Module):
    ...
    def __init__(self,use_viewdirs=True, name='', dim=3, c_dim=128,
                 input_ch_views =93,
                 hidden_size=256, n_blocks=3, leaky=False, sample_mode='bilinear',
        ...
        #=====================        
        if use_viewdirs:
            self.views_linears = nn.ModuleList([DenseLayer(input_ch_views + hidden_size,hidden_size//2)])
            self.feature_linear = DenseLayer(hidden_size,hidden_size)
            self.alpha_linear = DenseLayer(hidden_size,1)
            self.rgb_linear = DenseLayer(hidden_size//2,3)
    ...
    def forward(self, p,viewdir, c_grid=None):
        if self.c_dim != 0:
        ...
        if self.use_viewdirs:    
            ...
            embedded_viewdirs = self.embedder(viewdir)
        h = embedded_pts
        ...
        if self.use_viewdirs:
            ...
            h = torch.cat([feature, embedded_viewdirs], -1)
...
class MLP_no_xyz(nn.Module):
    ...
    def __init__(self,use_viewdirs, name='', dim=3, c_dim=128,
...
class NICE(nn.Module):
    ...
    def __init__(self, use_viewdirs,dim=3, c_dim=32,
        ...
        if coarse:
            self.coarse_decoder = MLP_no_xyz(use_viewdirs,
                name='coarse', dim=dim, c_dim=c_dim, color=False, hidden_size=hidden_size, grid_len=coarse_grid_len)

        self.middle_decoder = MLP(use_viewdirs,name='middle', dim=dim, c_dim=c_dim,input_ch_views=3, color=False,
                                  skips=[2], n_blocks=5, hidden_size=hidden_size,
                                  grid_len=middle_grid_len, pos_embedding_method=pos_embedding_method
                                  ) 
        self.fine_decoder = MLP(use_viewdirs,name='fine', dim=dim, c_dim=c_dim*2, input_ch_views=3,color=False,
                                skips=[2], n_blocks=5, hidden_size=hidden_size,
                                grid_len=fine_grid_len, concat_feature=True, pos_embedding_method=pos_embedding_method
                                )
        self.color_decoder = MLP(use_viewdirs,name='color', dim=dim, c_dim=c_dim,input_ch_views=3, color=True,
                                 skips=[2], n_blocks=5, hidden_size=hidden_size,
